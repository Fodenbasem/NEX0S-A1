...
export default function SessionsPage() {
  ...
  const revoke = async (id: string) => {
    try { await securityApi.revokeSession(id); qc.invalidateQueries({ queryKey: ["user-sessions"] }); toast.success("Session revoked"); }
    catch (e: any) { toast.error(e.message ?? "Failed"); }
  };
  const revokeAll = async () => {
    try { await securityApi.revokeAllOtherSessions(); qc.invalidateQueries({ queryKey: ["user-sessions"] }); toast.success("All other sessions revoked"); }
    catch (e: any) { toast.error(e.message ?? "Failed"); }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {sessionsQ.isLoading ? <Skeleton className="h-48 w-full" /> : sessions.length === 0 ? (
        <Card className="glass"><CardContent className="py-12 text-center text-muted-foreground">No sessions tracked yet. This session will appear after a moment.</CardContent></Card>
      ) : (
            ...
            {!isCurrent && onRevoke && (
              <AlertDialog>
                ...
                <AlertDialogContent>
                  ...
                  <AlertDialogFooter>
                    <AlertDialogCancel>Cancel</AlertDialogCancel>
                    <AlertDialogAction onClick={onRevoke} className="bg-destructive text-destructive-foreground">Revoke</AlertDialogAction>
                  </AlertDialogFooter>
                </AlertDialogContent>
              </AlertDialog>
            )}
