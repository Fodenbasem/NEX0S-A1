...
export const Route = createFileRoute("/_app/synthesis")({
  head: () => ({ meta: [{ title: "AI Synthesis Engine · NEX0S A1" }] }),
  component: Synthesis,
});

const fmtTs = (iso: string) => new Date(iso).toLocaleTimeString([], { hour12: false });

function deriveTerminal(project: Project | undefined, logs: any[], aiMsgs: any[]): TerminalLine[] {
  if (!project) return [];
  ...
  logs.slice(0, 30).forEach(l => out.push({
    ts: fmtTs(l.created_at),
    level: l.action.includes("complete") || l.action.includes("success") ? "ok" :
           l.action.includes("fail") ? "warn" : "info",
    text: `${l.action}${l.metadata ? "  " + JSON.stringify(l.metadata).slice(0, 80) : ""}`,
  }));
...
function Synthesis() {
  ...
  // Realtime: live logs for active project
  ...
  useEffect(() => {
    ...
    (async () => {
      const [{ data: l }, { data: a }] = await Promise.all([
        supabase.from("activity_logs").select("*").eq("project_id", active.id).order("created_at", { ascending: true }).limit(40),
        supabase.from("ai_requests").select("*").eq("project_id", active.id).order("created_at", { ascending: true }).limit(20),
      ]);
      if (cancelled) return;
      setLogs(l ?? []); setAiMsgs(a ?? []);
    })();
    const ch = supabase.channel(`synth-${active.id}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "activity_logs", filter: `project_id=eq.${active.id}` },
        (p) => setLogs(prev => [...prev, p.new]))
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "ai_requests", filter: `project_id=eq.${active.id}` },
        (p) => setAiMsgs(prev => [...prev, p.new]))
      .subscribe();
    return () => { cancelled = true; supabase.removeChannel(ch); };
  }, [active?.id]);

  const tree = useMemo(() => buildProjectTree(active?.stack ?? null), [active?.stack]);
  const terminal = useMemo(() => deriveTerminal(active, logs, aiMsgs), [active, logs, aiMsgs]);
    ...
    <div className="space-y-6">
      ...
      {projectsQ.isLoading ? <Skeleton className="h-96 w-full" /> : !active ? (
        ...
        <>
          ...
          <Card className="glass">
            <CardContent className="flex flex-wrap items-center justify-between gap-3 p-4">
              ...
              <Badge variant="outline" className="border-accent/40 bg-accent/10 text-accent">
                <Activity className="mr-1 h-3 w-3" /> live · realtime
              </Badge>
            </CardContent>
          </Card>

          {/* Top row: Pipeline + Terminal */}
          ...
          {/* Mid row: File tree + Architecture tabs */}
          <div className="grid gap-6 lg:grid-cols-5">
                  ...
                  <TabsContent value="flow" className="mt-3">
                    <div className="rounded-lg border border-border/40 bg-background/30 p-4">
                      ...
                      <div className="mt-3 font-mono text-[10.5px] leading-relaxed text-muted-foreground">
                        ...
                        <div>POST /functions/v1/ai-consultation → JWT → ai-gateway → SSE stream → INSERT ai_requests</div>
                      </div>
                    </div>
                  </TabsContent>
                </Tabs>
